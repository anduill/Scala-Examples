...
object CsvEncoder {
  ...
  implicit def genericEncoder[A, R] (implicit gen: Generic.Aux[A, R], enc: Lazy[CsvEncoder[R]] ): CsvEncoder[A] = CsvEncoder.instance {
    a => enc.value.encode (gen.to (a) )
  }

  implicit def listEncoder[A] (implicit enc: CsvEncoder[A] ): CsvEncoder[List[A]] = CsvEncoder.instance ( {
    l =>
      l.foldLeft (List[String] () ) {
        (res, el) => res ++ enc.encode (el)
      }
  })
}
...
//  }
//
//  sealed abstract class MCons extends MList {
//    self =>
//    val head: T
...
//  val tail = tl
//}
//  val nums = MCons (2, MCons (3, MNil () ) ): MCons {
//  type T = Int
//}
